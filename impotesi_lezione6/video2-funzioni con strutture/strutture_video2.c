...
//Funzione di stampa usando una struttura
void stampa_struct(const struct polinomio *poly){
    //Indice per il for
    int j;
...
int inserisci_elemento_struct(struct polinomio *poly){
    //Stringa 
    char s[5];
    //Intero letto
    int n;
...
int main(){
    ...
    //Così abbiamo realizzato una funzione che stampa un polinomio usando una struttura
    stampa_struct(p_pointer);

    // Esempio di utilizzo della funzione inserisci elemento struct
    inserisci_elemento_struct(p_pointer);
